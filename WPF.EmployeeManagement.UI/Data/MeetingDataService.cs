...
namespace WPF.EmployeeManagement.UI.Data
{
    ...
    {
        private readonly Func<MeetingDbContext> _dbContext;

        public MeetingDataService(Func<MeetingDbContext> dbContext)
        {
        ...
        public async Task<Meeting> GetMeetingById(int meetingID)
        {
            using (var context = _dbContext())
            {
                return await context.Meetings.SingleAsync(m => m.MeetingID == meetingID);
            }
        }
